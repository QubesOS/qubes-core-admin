...
import struct
import traceback
from typing import Union, Any, Literal, Generator
import uuid
...
def method(
    name,
    *,
    dest_adminvm: bool | None,
    wants_arg: bool | None,
    wants_payload: bool | None,
    scope: Literal["local", "global"] | None = None,
    read: bool = True,
    write: bool = False,
    execute: bool = False,
    endpoints: Generator[str, None, None] | None = None,
):
    ...
    :param bool dest_adminvm: if :py:obj:`True`, will barf if destination is \
        not AdminVM; If :py:obj:`False`, will barf if destination is AdminVM. \
        If :py:obj:`None`, checks are disabled.
    :param bool wants_arg: if :py:obj:`False`, will barf on non-empty \
        argument; also will not pass argument at all to the method. If \
        :py:obj:`True`, will barf on empty argument. If :py:obj:`None`, \
        checks are disabled.
    :param bool wants_payload: if :py:obj:`False`, will barf on non-empty \
        payload; also will not pass payload at all to the method. If \
        :py:obj:`True`, will barf on empty payload. If :py:obj:`None`, \
        checks are disabled.
    ...
    :param bool scope: if `global` or `local`, will include the service to the \
        global section of included policy directories.
    :param bool read: if :py:obj:`True` will add the service to the included \
        policy directories with read capabilities.
    :param bool write: if :py:obj:`True` will add the service to the included \
        policy directories with write capabilities.
    :param bool read: if :py:obj:`True` will add the service to the included \
        policy directories with execute capabilities.
    ...
    *self*), ``untrusted_payload``, which will contain the payload.
    ...
    def decorator(func):
        if wants_arg is not None and not isinstance(wants_arg, bool):
            raise ValueError("Invalid value for 'wants_arg'")
        if wants_payload is not None and not isinstance(wants_payload, bool):
            raise ValueError("Invalid value for 'wants_payload'")
        if dest_adminvm is not None and not isinstance(dest_adminvm, bool):
            raise ValueError("Invalid value for 'dest_adminvm'")
        if not isinstance(read, bool):
            raise ValueError("Invalid value for 'read'")
        if not isinstance(write, bool):
            raise ValueError("Invalid value for 'write'")
        if not isinstance(execute, bool):
            raise ValueError("Invalid value for 'execute'")
        if scope not in [None, "local", "global"]:
            raise ValueError("Invalid value for 'scope'")

        func.api_wants_arg = wants_arg
        func.api_wants_payload = wants_payload
        func.api_dest_adminvm = dest_adminvm
...
class AbstractQubesAPI:
    ...
    def execute(self, *, untrusted_payload):
        ...
        if handler.api_wants_payload is True:
            if untrusted_payload == b"":
                raise ProtocolError("expected payload")
            kwargs["untrusted_payload"] = untrusted_payload
        if handler.api_wants_payload is False and untrusted_payload != b"":
            raise ProtocolError("unexpected payload")
        if handler.api_wants_payload is None:
            kwargs["untrusted_payload"] = untrusted_payload
        del untrusted_payload

        if handler.api_wants_arg is True and not self.arg:
            raise ProtocolError("expected an argument")
        if handler.api_wants_arg is False:
            if self.arg:
                raise ProtocolError("unexpected argument")
            del self.arg

        if handler.api_dest_adminvm is True and self.dest.klass != "AdminVM":
            raise ProtocolError("expected destination to be AdminVM")
        if handler.api_dest_adminvm is False and self.dest.klass == "AdminVM":
            raise ProtocolError("expected destination to not be AdminVM")

        self._running_handler = asyncio.ensure_future(handler(self, **kwargs))
    ...
    def enforce_arg(
        self,
        wants: Union[list, tuple],
        short_reason: str = "",
    ) -> None:
        """Enforce argument to be absent or be in iterable."""
        assert isinstance(self.arg, str)
        assert short_reason
        if self.arg not in wants:
            raise PermissionDenied("Argument not in {!r}".format(short_reason))
    ...
    def validate_number(
        self, untrusted_number: bytes, name: str, allow_negative: bool = False
    ) -> int:
        allowed_chars = string.ascii_letters + string.digits + "-_. "
        assert all(c in allowed_chars for c in name)
        if not isinstance(untrusted_number, bytes):
            raise ProtocolError(
                "Expected {!r} to be of type bytes, got {!r}".format(
                    name, type(untrusted_number).__name__
                )
            )
        coefficient = 1
        if allow_negative and untrusted_number.startswith(b"-"):
            coefficient = -1
            untrusted_number = untrusted_number[1:]
        name_cap = name.capitalize()
        if not untrusted_number.isdigit():
            raise ProtocolError(name_cap + " contains non ASCII digits")
        if len(untrusted_number) >= 20:
            raise ProtocolError(name_cap + " is bigger than 19 decimal digits")
        if untrusted_number[0] == 48 and untrusted_number != b"0":
            raise ProtocolError(name_cap + " contains spurious leading zeros")
        return int(untrusted_number) * coefficient
