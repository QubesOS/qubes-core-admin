        ...
        try:
            ...
            notes = "".join(
                [
                    (
                        c
                        # first we use Python internal set of safe chars
                        # which includes \r \x0b & \x0c
                        if c in string.printable
                        # then we revert to advanced unicode sanitisation
                        or libqubespure.qubes_pure_string_safe_for_display(
                            f"{c}".encode(), 0
                        )
                        else "_"
                    )
