...
parser.add_argument("src", metavar="SRC", help="source qube")
parser.add_argument("method", metavar="METHOD", help="method name")
parser.add_argument("dest", metavar="DEST", help="destination qube")
parser.add_argument(
    "arg", metavar="ARGUMENT", nargs="?", default="", help="argument to method"
)
parser.add_argument(
    "--max-bytes",
    dest="max_bytes",
...
def main(args=None):
    ...
    if args.max_bytes is not None:
        if args.max_bytes > MAX_PAYLOAD_SIZE:
            parser.error(
                "Maximum payload too large (max {})".format(max_payload_size)
            )
            return 1
    ...
    if args.payload:
        if args.single_line:
            ...
            for _ in range(0, max_payload_size):
                ...
                parser.error(
                    "No newline found in first {} bytes".format(
                        max_payload_size
                    )
                )
            ...
            if len(payload) > max_payload_size:
                parser.error(
                    "Payload too long (max {})".format(max_payload_size)
                )
                # make sure to terminate, even if parser.error() would return
                # for some reason
                return 1
    ...
    for signame in ("SIGINT", "SIGTERM"):
        loop.add_signal_handler(
            getattr(signal, signame), sighandler, loop, signame, coro
        )
