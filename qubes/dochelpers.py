...
class VersionCheck(docutils.parsers.rst.Directive):
    ...
    def run(self):
        ...
        text = " ".join(
            """This manual page was written for version **{}**, but
        ...
        self.state.nested_parse(
            docutils.statemachine.StringList([text]), self.content_offset, node
        )
        return [node]
...
class ManpageCheckVisitor(docutils.nodes.SparseNodeVisitor):
    ...
    def __init__(self, app, command, document):
        ...
        for action in parser._actions:
            ...
            if issubclass(
                action.__class__, qubes.tools.AliasedSubParsersAction
            ):
    ...
    def visit_section(self, node):
        ...
        if section_title == OPTIONS_TITLE:
            options_visitor = OptionsCheckVisitor(
                self.command, self.options, self.document
            )
            node.walkabout(options_visitor)
            options_visitor.check_undocumented_arguments()
...
def setup(app):
    ...
    fdesc = sphinx.util.docfields.GroupedField(
