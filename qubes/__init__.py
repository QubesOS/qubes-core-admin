...
class Label:
    ...
    def __repr__(self):
        return "{}({!r}, {!r}, {!r})".format(
            self.__class__.__name__, self.index, self.color, self.name
        )
    ...
    def icon_path(self):
        ...
        """
        return (
            os.path.join(qubes.config.system_path["qubes_icon_dir"], self.icon)
            + ".png"
        )
...
class property:  # pylint: disable=redefined-builtin,invalid-name
    ...
    def __init__(
        ...
        *,
        ...
        self._saver = (
            ...
            else (lambda self, prop, value: str(value))
        )
        self.type = type
    ...
    def __get__(self, instance, owner):
        ...
        if not isinstance(instance, PropertyHolder):
            raise AttributeError(
                "qubes.property should be used on "
    ...
    def __set__(self, instance, value):
        ...
        if has_oldvalue:
            instance.fire_event(
                "property-pre-set:" + self.__name__,
                pre_event=True,
                name=self.__name__,
                newvalue=value,
                oldvalue=oldvalue,  # pylint: disable=possibly-used-before-assignment
            )
        else:
            instance.fire_event(
                "property-pre-set:" + self.__name__,
    ...
    def __delete__(self, instance):
        ...
        if has_oldvalue:
            instance.fire_event(
                "property-pre-reset:" + self.__name__,
                pre_event=True,
                name=self.__name__,
                oldvalue=oldvalue,
            )
            ...
            instance.fire_event(
                "property-pre-del:" + self.__name__,
                pre_event=True,
                name=self.__name__,
                oldvalue=oldvalue,
            )
            try:
                ...
                pass
            instance.fire_event(
                "property-reset:" + self.__name__,
                name=self.__name__,
                oldvalue=oldvalue,
            )
            # deprecated, to be removed in Qubes 5.0
            instance.fire_event(
                "property-del:" + self.__name__,
                name=self.__name__,
                oldvalue=oldvalue,
            )
            ...
            instance.fire_event(
                "property-reset:" + self.__name__, name=self.__name__
            )
            # deprecated, to be removed in Qubes 5.0
            instance.fire_event(
                "property-del:" + self.__name__, name=self.__name__
            )

    def __repr__(self):
        default = (
            " default={!r}".format(self._default)
    ...
    def sanitize(self, *, untrusted_newvalue):
        ...
        """
        try:
            untrusted_newvalue = untrusted_newvalue.decode(
                "ascii", errors="strict"
            )
        except UnicodeDecodeError:
            raise qubes.exc.QubesValueError("Non-ASCII bytes in property value")
...
class PropertyHolder(qubes.events.Emitter):
    ...
    def property_get_def(cls, prop):
        ...
        raise AttributeError("No property {!r} found in {!r}".format(prop, cls))
    ...
    def clone_properties(self, src, proplist=None):
        ...
        if proplist is None:
            proplist = [prop for prop in self.property_list() if prop.clone]
        else:
            proplist = [
                prop
                for prop in self.property_list()
