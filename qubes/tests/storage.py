...
class TC_00_Pool(QubesTestCase):
    ...
    def setUp(self):
        ...
        self.basedir_patch = unittest.mock.patch(
            "qubes.config.qubes_base_dir", "/tmp/qubes-test-basedir"
        )
    ...
    def test_002_get_pool_klass(self):
        ...
        self.assertTrue(
            isinstance(result, FilePool) or isinstance(result, ReflinkPool)
        )
    ...
    def test_004_add_remove_pool(self):
        ...
        self.loop.run_until_complete(
            self.app.add_pool(
                name=pool_name, driver="file", dir_path="/tmp/asdjhrp89132"
            )
        )
    ...
    def test_005_remove_used(self):
        ...
        pool = self.loop.run_until_complete(
            self.app.add_pool(name=pool_name, driver="file", dir_path=dir_path)
        )
        self.addCleanup(shutil.rmtree, dir_path)
        vm = self.app.add_new_vm(
            "StandaloneVM", label="red", name=self.make_vm_name("vm")
        )
    ...
    def test_006_pool_usage_qubespool(self):
        ...
        self.assertIsNotNone(
            usage_details["data_size"], "Data size incorrectly reported as None"
        )
        self.assertIsNotNone(
            usage_details["data_usage"],
    ...
    def test_007_pool_kernels_usage(self):
        ...
        pool = self.loop.run_until_complete(
            self.app.add_pool(
                name=pool_name, driver="linux-kernel", dir_path=dir_path
            )
        )
        self.assertTrue(self.assertPoolExists(pool_name))
        ...
        self.assertEqual(
            usage_details, {}, "Kernels pool should not report usage details."
        )
