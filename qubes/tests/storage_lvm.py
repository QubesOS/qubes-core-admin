...
def lvm_pool_exists(volume_group, thin_pool):
    ...
    path = "/dev/mapper/{!s}-{!s}".format(
        volume_group.replace("-", "--"), thin_pool.replace("-", "--")
    )
...
class ThinPoolBase(qubes.tests.QubesTestCase):
    ...
    @classmethod
    def setUpClass(
        cls, pool_class=ThinPool, volume_class=ThinVolume, pool_conf=None
    ):
    ...
    def _find_pool(self, volume_group, thin_pool):
        ...
        pools = [
            p
            for p in self.app.pools.values()
...
class TC_00_ThinPool(ThinPoolBase):
    ...
    def setUp(self, **kwargs):
        ...
        self.app = qubes.Qubes.create_empty_store(
            store=xml_path,
            clockvm=None,
            updatevm=None,
            offline_mode=True,
    ...
    def __assertPathMatch(self, path1, path2):
        ...
        self.assertEqual(
            path1[12:], path2[5:].replace("-", "--").replace("/", "-")
        )
    ...
    def test_008_commit(self):
        ...
        self.assertTrue(
            path.startswith(
                "/dev/mapper/" + volume.vid.replace("-", "--").replace("/", "-")
            ),
    ...
    def test_009_interrupted_commit(self):
        ...
        self.__assertPathMatch(volume.path, "/dev/" + volume.vid + revisions[1])
        expected_revisions = {
    ...
    def test_011_migration2(self):
        ...
        self.assertTrue(os.path.exists("/dev/" + volume.vid))
        self.assertEqual(
            self._get_lv_uuid(volume._vid_current), orig_uuids["-snap"]
        )
    ...
    def test_012_migration3(self):
        ...
        self.assertEqual(
            self._get_lv_uuid(volume._vid_snap), orig_uuids["-snap"]
        )
    ...
    def test_030_import_data(self):
        ...
        self.assertEqual(
            current_uuid, self._get_lv_uuid(volume.vid + "-" + revision)
        )
    ...
    def test_032_import_volume_same_pool(self):
        ...
        vm = qubes.tests.storage.TestVM(self)
        source_volume = self.app.get_pool(self.pool.name).init_volume(
            vm, config
        )
        ...
        with unittest.mock.patch("time.time") as mock_time:
            ...
            self.loop.run_until_complete(volume.import_volume(source_volume))
    ...
    def test_040_volatile(self):
        ...
        self.assertTrue(
            self.loop.run_until_complete(volume.verify()),
            "volume cannot be verified?",
        )
        self.loop.run_until_complete(volume.create())
        self.assertTrue(
            self.loop.run_until_complete(volume.verify()),
        ...
        self.assertFalse(
            volume.pool.ephemeral_volatile,
            "pool enabled ephemeral volatile volumes?",
        )
        volume.pool.ephemeral_volatile = True
        self.assertTrue(
            volume.ephemeral, "pool changes should be reflected in the volume"
        )
        ...
        self.__assertPathMatch(path, "/dev/" + volume.vid)
        self.assertFalse(
            os.path.exists(path), "volume path %r exists but should not!" % path
        )
        ...
        self.assertTrue(
            os.path.exists(path),
            "volume path %r does not exist but should!" % path,
        )
        self.assertTrue(
            os.path.exists(encrypted_path),
            "encrypted path %r does not exist but should!" % encrypted_path,
        )
        ...
        self.assertTrue(
            bdev.path.endswith("-vm--test--inst--appvm--volatile@crypt")
        )
        self.assertEqual(
            bdev.path,
            "/dev/mapper/"
            + "-".join(i.replace("-", "--") for i in volume.vid.split("/"))
            + "@crypt",
        )
        self.assertTrue(
            encrypted_path.startswith("/dev/mapper/"),
            "bad path %r" % encrypted_path,
        )
        self.assertTrue(
            encrypted_path.endswith("@crypt"),
            "bad encrypted path %r" % encrypted_path,
        )
        vol_uuid = self._get_lv_uuid(path)
        self.loop.run_until_complete(volume.start_encrypted(encrypted_path))
        self.assertTrue(
            os.path.exists(path),
            "time 2: volume path %r does not exist but should!" % path,
        )
        vol_uuid2 = self._get_lv_uuid(path)
        self.assertNotEqual(vol_uuid, vol_uuid2)
        self.loop.run_until_complete(
            volume.stop_encrypted(
                volume.encrypted_volume_path(vm.name, config["name"])
            )
        )
        self.assertFalse(
            os.path.exists(path),
            "after stop: volume path %r exists but should not!" % path,
        )
...
class TC_01_ThinPool(ThinPoolBase, qubes.tests.SystemTestCase):
    ...
    def test_004_import(self):
        ...
        vm = self.app.add_new_vm(
            qubes.vm.templatevm.TemplateVM, name=name, label="red"
        )
    ...
    def test_006_name_suffix_parse(self):
        ...
        vm2 = self.app.add_new_vm(
            cls=qubes.vm.appvm.AppVM,
            name=self.make_vm_name("appvm-root"),
            label="red",
        )
        self.loop.run_until_complete(
            asyncio.wait(
