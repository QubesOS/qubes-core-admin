...
class TC_01_FileVolumes(qubes.tests.QubesTestCase):
        ...
        if qubes.tests.in_git:
            self.patches.append(
                unittest.mock.patch(
                    ...
                    os.path.join(
                        _dir, "../../linux/system-config/create-snapshot"
                    ),
                )
            )
            self.patches.append(
                unittest.mock.patch(
                    ...
                    os.path.join(
                        _dir, "../../linux/system-config/destroy-snapshot"
                    ),
                )
            )
    ...
    def test_001_snapshot_volume(self):
        ...
        source = self.app.get_pool(self.POOL_NAME).init_volume(
            template_vm, source_config
        )
        config = {
            "name": "root",
            "pool": self.POOL_NAME,
    ...
    def test_005_appvm_volumes(self):
        ...
        template_dir = os.path.join(
            self.POOL_DIR, "vm-templates", vm.template.name
        )
        ...
        expected = vm_dir + "/private.img:" + vm_dir + "/private-cow.img"
    ...
    def test_007_resize_running(self):
        ...
        ) as mock_subprocess, unittest.mock.patch(
            "subprocess.check_output"
    ...
    def assertVolumePath(self, vm, dev_name, expected, rw=True):
        ...
        volumes = vm.volumes
        self.loop.run_until_complete(
            qubes.utils.coro_maybe(volumes[dev_name].create())
        )
        self.loop.run_until_complete(
            qubes.utils.coro_maybe(volumes[dev_name].start())
        )
        b_dev = volumes[dev_name].block_device()
        self.assertEqual(b_dev.rw, rw)
        self.loop.run_until_complete(
            qubes.utils.coro_maybe(volumes[dev_name].stop())
        )
...
class TC_03_FilePool(qubes.tests.QubesTestCase):
    ...
    def test_004_usage(self):
        ...
        statvfs = os.statvfs(self.POOL_DIR)
        self.assertEqual(
            usage, statvfs.f_frsize * (statvfs.f_blocks - statvfs.f_bfree)
        )
    ...
    def test_011_appvm_file_images(self):
        ...
        self.assertEqualAndExists(
            vm.volumes["volatile"].path, expected_volatile_path
        )
    ...
    def test_013_template_file_images(self):
        ...
        expected_root_path = "%s:%s" % (
            expected_root_origin_path,
            expected_root_cow_path,
        )
        self.assertEqualAndExists(
            vm.volumes["root"].path, expected_root_origin_path
        )

        expected_private_path = os.path.join(expected_vmdir, "private.img")
        self.assertEqualAndExists(
            vm.volumes["private"].path, expected_private_path
        )
