...
"""
    ...
    RPC_FILE_PATH = os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "../../qubes-rpc/admin.vm.volume.Import"
        )
    )
    ...
    def setUp(self):
        ...
        with open(
            os.path.join(self.tmpdir, "qubesd-query"), "w"
        ) as qubesd_query_f:
    ...
    def mockMethod(self, method, response):
        with open(
            os.path.join(self.tmpdir, "response-" + method), "wb"
        ) as response_f:
    ...
    def assertMethodCalled(self, method, arg, expected_payload=b""):
        try:
            with open(
                os.path.join(self.tmpdir, "command-" + method), "rb"
            ) as command_f:
                command = command_f.read()
            with open(
                os.path.join(self.tmpdir, "payload-" + method), "rb"
            ) as payload_f:
        ...
        self.assertListEqual(
            command.decode().split(),
            [
                "-c",
                "/var/run/qubesd.internal.sock",
                "remote",
                method,
                "target",
                arg,
    ...
    def setup_import(self, size):
        ...
        self.mockMethod("internal.vm.volume.ImportEnd", b"\x30\x00import-end")
    ...
    def run_rpc(self, command, arg, data):
        with open(os.path.join(self.tmpdir, "data"), "w+b") as data_f:
            ...
            output = subprocess.check_output(
                [command, arg], env=env, cwd=self.tmpdir, stdin=data_f
            )
    ...
    def test_00_import(self):
        ...
        self.assertMethodCalled("internal.vm.volume.ImportEnd", "volume", b"ok")
    ...
    def test_01_import_with_size(self):
        ...
        self.assertMethodCalled(
            "internal.vm.volume.ImportBegin", "volume", str(size).encode()
        )
        self.assertMethodCalled("internal.vm.volume.ImportEnd", "volume", b"ok")
        self.assertFileData(self.target, data)
    ...
    def test_02_import_not_enough_data(self):
        ...
        self.assertMethodCalled(
            ...
            b"fail\n"
            + (
                "not enough data (copied {} bytes, expected {} bytes)".format(
                    len(data), size
                ).encode()
            ),
        )
    ...
    def test_03_import_too_much_data(self):
        ...
        self.assertMethodCalled(
            ...
            "volume",
            b"fail\n"
            + ("too much data (expected {} bytes)".format(size).encode()),
        )
