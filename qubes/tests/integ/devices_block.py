...
class TC_00_List(qubes.tests.SystemTestCase):
    ...
    def test_000_list_loop(self):
        ...
        self.run_script(
            ...
            "losetup -f {path}; "
            "udevadm settle".format(path=self.img_path),
            user="root",
        )
    ...
    def test_001_list_loop_mounted(self):
        ...
        for dev in dev_list:
            if dev.serial == self.img_path:
                self.fail(
                    "Device {} ({}) should not be listed because is mounted".format(
                        dev, self.img_path
                    )
                )
...
class AttachMixin:
    ...
    def setUp(self):
        ...
        self.backend = self.app.add_new_vm(
            "AppVM", label="red", name=self.make_vm_name("back")
        )
        self.loop.run_until_complete(self.backend.create_on_disk())
        self.frontend = self.app.add_new_vm(
            "AppVM", label="red", name=self.make_vm_name("front")
        )
        self.loop.run_until_complete(self.frontend.create_on_disk())
        self.app.save()
        exc = self.loop.run_until_complete(
            asyncio.gather(
                self.backend.start(),
                self.frontend.start(),
        ...
        for dev in dev_list:
            ...
            self.fail(
                "Device for {} in {} not found".format(
...
def create_testcases_for_templates():
    yield from qubes.tests.create_testcases_for_templates(
        "TC_00_List", TC_00_List, module=sys.modules[__name__]
    )
    yield from qubes.tests.create_testcases_for_templates(
        "TC_10_Attach",
        AttachMixin,
