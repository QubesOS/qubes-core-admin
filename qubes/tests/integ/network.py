...
class VmNetworkingMixin(object):
    ...
    def _networking_paused_from_none_to_existent(
        ...
        self.assertIsNone(
            self.testvm1.features.get("deferred-netvm-force", None)
        )
    ...
    def _networking_paused_restart_netvm(
        ...
        self.assertTrue(self.testvm1.features.get("deferred-netvm-force", None))
    ...
    def test_010_simple_proxyvm(self):
        ...
        self.assertEqual(
            self.run_cmd(self.proxy, self.ping_ip),
            0,
            "Ping by IP from ProxyVM failed",
        )
        self.assertEqual(
            self.run_cmd(self.proxy, self.ping_name),
            0,
            "Ping by name from ProxyVM failed",
        )
        self.assertEqual(
            self.run_cmd(self.testvm1, self.ping_ip),
            0,
            "Ping by IP from AppVM failed",
        )
        self.assertEqual(
            self.run_cmd(self.testvm1, self.ping_name),
            0,
            "Ping by IP from AppVM failed",
        )
    ...
    def test_020_simple_proxyvm_nm(self):
        ...
        self.assertEqual(
            self.run_cmd(self.testvm1, self.ping_name), 0, "Ping by name failed"
        )

        # reconnect to make sure that device was configured by NM
        self.assertEqual(
            self.run_cmd(
                self.proxy, "nmcli device disconnect eth0", user="user"
            ),
            0,
            "Failed to disconnect eth0 using nmcli",
        )
        ...
        self.assertEqual(
            ...
            0,
            "Failed to connect eth0 using nmcli",
        )
        self.assertEqual(
            self.run_cmd(self.proxy, "nm-online", user="user"),
            0,
        ...
        self.assertEqual(
            subprocess.call(
                [
                    ...
                    "search",
                    "--class",
                    "{}:nm-applet".format(self.proxy.name),
                ],
                stdout=subprocess.DEVNULL,
            ),
            0,
            "nm-applet window not found",
        )
        self.assertEqual(
            self.run_cmd(self.testvm1, self.ping_ip),
            0,
    ...
    def test_030_firewallvm_firewall(self):
        ...
        server = self.loop.run_until_complete(
            self.testnetvm.run("socat TCP-LISTEN:1234,fork EXEC:/bin/uname")
        )
        ...
        try:
            ...
            self.assertNotEqual(
                self.run_cmd(self.testvm1, client_cmd),
                0,
    ...
    def test_031_firewall_dynamic_block(self):
        ...
        server1 = self.loop.run_until_complete(
            self.testnetvm.run(
                "socat -u TCP-LISTEN:1234 -", stdout=subprocess.PIPE
            )
        )
        server2 = self.loop.run_until_complete(
            self.testnetvm.run(
        ...
        try:
            ...
            client1 = self.loop.run_until_complete(
                self.testvm1.run(client_cmd, stdin=subprocess.PIPE)
            )
            ...
            client2 = self.loop.run_until_complete(
                self.testvm1.run(client_cmd, stdin=subprocess.PIPE)
            )
    ...
    def test_040_inter_vm(self):
        ...
        self.loop.run_until_complete(
            asyncio.gather(
                self.start_vm(self.testvm1), self.start_vm(self.testvm2)
            )
        )

        self.assertNotEqual(
            self.run_cmd(
                self.testvm1, self.ping_cmd.format(target=self.testvm2.ip)
            ),
            0,
        )
        ...
        self.assertNotEqual(
            self.run_cmd(
                self.testvm2, self.ping_cmd.format(target=self.testvm1.ip)
            ),
            0,
        )
    ...
    def test_050_spoof_ip(self):
        ...
        if retcode == 127:
            ...
            raise AssertionError("{} failed with: {}".format(cmd, retcode))
        self.loop.run_until_complete(
    ...
    def test_111_dynamic_detach_attach(self):
        ...
        # wait for it to settle down
        self.loop.run_until_complete(
            self.testvm1.run_for_stdio("udevadm settle")
        )
        self.testvm1.netvm = self.testnetvm
        ping_cmd = f"until ping -W 1 -n -c 1 {self.test_ip}; do sleep 1; done"
        self.assertEqual(self.run_cmd(self.testvm1, ping_cmd, timeout=10), 0)
    ...
    def test_114_reattach_after_provider_crash(self):
        ...
        p = self.loop.run_until_complete(
            self.proxy.run("echo c > /proc/sysrq-trigger", user="root")
        )
        ...
        while self.proxy.is_running():
            self.loop.run_until_complete(asyncio.sleep(1))
            timeout -= 1
            self.assertGreater(timeout, 0, "timeout waiting for crash cleanup")
        self.loop.run_until_complete(self.start_vm(self.proxy))
        self.assertEqual(self.run_cmd(self.testvm1, self.ping_ip), 0)
    ...
    def test_203_fake_ip_inter_vm_allow(self):
        ...
        self.assertNotEqual(
            stdout.decode().split()[-2], "0", "Packets didn't managed to the VM"
        )
