...
class TC_00_BackupCompatibility(
    ...
    def create_v1_files(self, r2b2=False):
        ...
        self.create_whitelisted_appmenus(
            self.fullpath("appvms/test-work/whitelisted-appmenus.list")
        )
        ...
        self.create_whitelisted_appmenus(
            self.fullpath("appvms/test-standalonevm/whitelisted-appmenus.list")
        )
        self.create_private_img(
            self.fullpath("appvms/test-standalonevm/private.img")
        )
        self.create_sparse(
            self.fullpath("appvms/test-standalonevm/root.img"), 10 * 2**30
        )
        self.fill_image(
            self.fullpath("appvms/test-standalonevm/root.img"),
            100 * 1024 * 1024,
            True,
        )
        ...
        for k_file in ["initramfs", "vmlinuz", "modules.img"]:
            self.fill_image(
                self.fullpath("appvms/test-standalonevm/kernels/" + k_file),
                10 * 1024 * 1024,
            )
        ...
        subprocess.check_call(
            [
                "/bin/cp",
                "-a",
                self.fullpath("appvms/test-work"),
        ...
        if r2b2:
            subprocess.check_call(
                [
                    "/bin/cp",
                    "-a",
                    self.fullpath("appvms/test-standalonevm"),
        ...
        self.create_whitelisted_appmenus(
            self.fullpath("servicevms/test-testproxy/whitelisted-appmenus.list")
        )
        self.create_private_img(
        ...
        self.create_sparse(
            self.fullpath("vm-templates/test-template-clone/root-cow.img"),
            10 * 2**30,
        )
        self.create_sparse(
            self.fullpath("vm-templates/test-template-clone/root.img"),
            10 * 2**30,
        )
        self.fill_image(
            self.fullpath("vm-templates/test-template-clone/root.img"),
        ...
        self.create_volatile_img(
            self.fullpath("vm-templates/test-template-clone/volatile.img")
        )
        subprocess.check_call(
            [
                "/bin/tar",
                ...
                "-C",
                self.fullpath("vm-templates/test-template-clone"),
                "volatile.img",
            ]
        )
        self.create_whitelisted_appmenus(
        ...
        self.create_appmenus(
            ...
            APPTEMPLATE_R2B2.replace("%VMNAME%", "test-template-clone").replace(
                "%VMDIR%", self.fullpath("vm-templates/test-template-clone")
            ),
            appmenus_list,
        )
    ...
    def append_backup_stream(self, f_name, stream, basedir=None):
        ...
        subprocess.check_call(
            ["tar", "-cO", "--posix", "-C", basedir, f_name], stdout=stream
        )

    def handle_v3_file(
        self, f_name, subdir, stream, compressed=True, encrypted=True
    ):
        ...
        tar_cmdline = [
            ...
            "--sparse",
            "-C",
            self.fullpath(os.path.dirname(f_name)),
            "--xform",
            "s:^%s:%s\\0:" % (os.path.basename(f_name), subdir),
        ...
        subprocess.check_call(
            [
                ...
                "-",
                os.path.join(stage1_dir, os.path.basename(f_name + ".")),
            ],
            stdin=data,
        )
        ...
        for part in sorted(os.listdir(stage1_dir)):
            if not re.match(
                r"^{}.[0-9][0-9][0-9]$".format(os.path.basename(f_name)), part
            ):
                continue
            part_with_dir = os.path.join(subdir, part)
            self.calculate_hmac(os.path.join("stage1", part_with_dir))
            self.append_backup_stream(
                part_with_dir, stream, basedir=self.fullpath("stage1")
            )
            self.append_backup_stream(
                part_with_dir + ".hmac", stream, basedir=self.fullpath("stage1")
            )
    ...
    def create_v3_backup(self, encrypted=True, compressed=True):
        ...
        f.write(
            BACKUP_HEADER_R2.format(
                encrypted=str(encrypted), compressed=str(compressed)
            )
        )
        f.close()
        ...
        if encrypted:
            ...
            for vmname, subdir in MANGLED_SUBDIRS_R2.items():
                qubesxml = re.sub(
                    r"[a-z-]*/{}".format(vmname), subdir, qubesxml
                )
            f.write(qubesxml)
        else:
            f.write(QUBESXML_R2)
        ...
        self.handle_v3_file(
            "qubes.xml", "", output, encrypted=encrypted, compressed=compressed
        )
    ...
    def test_210_r2(self):
        ...
        self.assertRestored(
            "test-testproxy", klass=qubes.vm.appvm.AppVM, **testproxy_props
        )
        self.assertRestored(
            "test-work",
...
class TC_01_BackupCompatibilityIntoLVM(TC_00_BackupCompatibility):
    ...
    def restore_backup(self, source=None, appvm=None, options=None, **kwargs):
        if options is None:
            options = {}
        options["override_pool"] = self.pool.name
        super(TC_01_BackupCompatibilityIntoLVM, self).restore_backup(
            source, appvm, options, **kwargs
        )
