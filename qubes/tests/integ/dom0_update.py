...
%commit
        ...
        subprocess.check_call(
            gpg_opts + ["-a", "--export", "--output", export_to]
        )
        p = subprocess.Popen(
            gpg_opts + ["--with-colons", "--list-keys"], stdout=subprocess.PIPE
        )
...
"""
    ...
    def setUp(self):
        ...
        self.app.save()
        subprocess.call(["rpm", "-e", self.pkg_name], stderr=subprocess.DEVNULL)
    ...
    def tearDown(self):
        ...
        subprocess.call(["rpm", "-e", self.pkg_name], stderr=subprocess.DEVNULL)
        subprocess.call(
            ["rpm", "-e", "gpg-pubkey-{}".format(self.keyid)],
...
%files
        ...
        pkg_path = os.path.join(
            dir, "x86_64", "{}-{}-1.x86_64.rpm".format(name, version)
        )
    ...
    def send_pkg(self, filename):
        ...
        try:
            ...
            if e.returncode == 127:
                self.skipTest(
                    "createrepo_c not installed in template {}".format(
                        self.template
                    )
                )
            else:
                self.skipTest(
                    "createrepo_c failed with code {}, "
...
""",
    ...
    def test_000_update(self):
        ...
        self.assertEqual(
            retcode,
            1,
            "Package {}-1.0 still installed after "
        ...
        self.assertEqual(
            retcode,
            0,
            "Package {}-2.0 not installed after "
            "update".format(self.pkg_name),
        )
        self.assertFalse(
    ...
    def test_005_update_flag_clear(self):
        ...
        with open(logpath) as f:
            ...
            self.assertFalse(
                "Errno" in dom0_update_output
                or "Couldn't" in dom0_update_output,
                "qubes-dom0-update reported an error: {}".format(
                    dom0_update_output
                ),
            )
    ...
    def test_010_instal(self):
        ...
        self.assertEqual(
            retcode, 0, "Package {}-1.0 not installed".format(self.pkg_name)
        )

    def test_020_install_wrong_sign(self):
        subprocess.call(["rpm", "-e", "gpg-pubkey-{}".format(self.keyid)])
        filename = self.create_pkg(self.tmpdir, self.pkg_name, "1.0")
        self.send_pkg(filename)
        ...
        if not proc.returncode:
            ...
            with open(logpath) as f_log:
                self.fail(
                    "qubes-dom0-update unexpectedly succeeded: " + f_log.read()
                )
        del proc
...
class TC_10_QvmTemplateMixin(object):
    ...
    def test_000_template_list(self):
        retcode, stdout, stderr = self.run_qvm_template(
            "qvm-template",
            *self.common_args,
    ...
    def test_010_template_install(self):
        ...
        got_hostname, _ = self.loop.run_until_complete(
            tpl.run_for_stdio("uname -n")
        )
...
def create_testcases_for_templates():
    yield from qubes.tests.create_testcases_for_templates(
        "TC_00_Dom0Upgrade",
        TC_00_Dom0UpgradeMixin,
        qubes.tests.SystemTestCase,
        module=sys.modules[__name__],
    )
    yield from qubes.tests.create_testcases_for_templates(
        "TC_10_QvmTemplate",
        TC_10_QvmTemplateMixin,
        qubes.tests.SystemTestCase,
        module=sys.modules[__name__],
    )
    yield from qubes.tests.create_testcases_for_templates(
        "TC_11_QvmTemplateMgmtVM",
        TC_11_QvmTemplateMgmtVMMixin,
        qubes.tests.SystemTestCase,
        module=sys.modules[__name__],
    )
