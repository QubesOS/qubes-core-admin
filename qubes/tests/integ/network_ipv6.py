...
class VmIPv6NetworkingMixin(VmNetworkingMixin):
    ...
    def test_510_ipv6_simple_proxyvm(self):
        ...
        self.assertEqual(
            self.run_cmd(self.proxy, self.ping6_ip),
            0,
            "Ping by IP from ProxyVM failed",
        )
        self.assertEqual(
            self.run_cmd(self.proxy, self.ping6_name),
            0,
            "Ping by name from ProxyVM failed",
        )
        self.assertEqual(
            self.run_cmd(self.testvm1, self.ping6_ip),
            0,
            "Ping by IP from AppVM failed",
        )
        self.assertEqual(
            self.run_cmd(self.testvm1, self.ping6_name),
            0,
            "Ping by IP from AppVM failed",
        )
    ...
    def test_530_ipv6_firewallvm_firewall(self):
        ...
        server = self.loop.run_until_complete(
            self.testnetvm.run("socat TCP6-LISTEN:1234,fork EXEC:/bin/uname")
        )
        ...
        try:
            ...
            self.assertNotEqual(
                self.run_cmd(self.testvm1, client6_cmd),
                0,
            ...
            self.assertEqual(
                self.run_cmd(self.testvm1, client4_cmd),
                0,
    ...
    def test_540_ipv6_inter_vm(self):
        ...
        self.assertNotEqual(
            self.run_cmd(
                self.testvm1, self.ping_cmd.format(target=self.testvm2.ip6)
            ),
            0,
        )
        ...
        self.assertNotEqual(
            self.run_cmd(
                self.testvm2, self.ping_cmd.format(target=self.testvm1.ip6)
            ),
            0,
        )
