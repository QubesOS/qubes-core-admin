...
class TC_00_RestoreInDispVM(qubes.tests.integ.backup.BackupTestsMixin):
    ...
    def test_000_basic_backup(self):
        ...
        try:
            ...
            self.make_backup(
                vms, target="/var/tmp/backup directory", target_vm=self.backupvm
            )
        ...
        self.restore_backup(source=backup_path, appvm=self.backupvm.name)
        self.assertCorrectlyRestored(vms_info, orig_hashes)


def create_testcases_for_templates():
    return qubes.tests.create_testcases_for_templates(
        "TC_10_RestoreInDispVM",
        TC_00_RestoreInDispVM,
