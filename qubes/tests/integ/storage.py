...
class StorageTestMixin(object):
    ...
    async def _test_000_volatile(self):
        ...
        await self.vm1.run_for_stdio(
            "cmp -n 16K /dev/xvde /dev/zero", user="root"
        )
        ...
        await self.vm1.run_for_stdio(
            "! dd if=/dev/xvde bs=1 skip=1M count=7 | grep test123", user="root"
        )
    ...
    async def _test_002_read_only(self):
        ...
        with self.assertRaises(subprocess.CalledProcessError):
            await self.vm1.run_for_stdio(
                "echo test123 > /dev/xvde", user="root"
            )
    ...
    def test_005_size_after_clone(self):
        """Test snapshot volume non-persistence"""
        return self.loop.run_until_complete(self._test_005_size_after_clone())
...
class StorageFile(StorageTestMixin, qubes.tests.SystemTestCase):
    def init_pool(self):
        ...
        self.pool = self.loop.run_until_complete(
            self.app.add_pool(
                dir_path=self.dir_path, name="test-pool", driver="file"
            )
        )
        os.makedirs(
            os.path.join(self.dir_path, "appvms", self.vm1.name), exist_ok=True
        )
        os.makedirs(
            os.path.join(self.dir_path, "appvms", self.vm2.name), exist_ok=True
        )
...
class StorageReflinkMixin(StorageTestMixin):
    ...
    def init_pool(self, fs_type, **kwargs):
        ...
        qubes.tests.storage_reflink.mkdir_fs(
            dir_path, fs_type, cleanup_via=self.addCleanup
        )
        self.pool = self.loop.run_until_complete(
            self.app.add_pool(
                name=name, dir_path=dir_path, driver="file-reflink", **kwargs
            )
        )
