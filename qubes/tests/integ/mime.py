...
@unittest.skipUnless(
    spawn.find_executable("xprop")
    and spawn.find_executable("xdotool")
    and spawn.find_executable("wmctrl"),
...
class TC_50_MimeHandlers:
    def setUp(self):
        ...
        self.source_vm = self.app.add_new_vm(
            "AppVM",
        ...
        self.target_vm = self.app.add_new_vm(
            "AppVM",
            template=self.template,
        ...
        self.loop.run_until_complete(
            asyncio.gather(self.source_vm.start(), self.target_vm.start())
        )
    ...
    def get_window_class(self, winid, dispvm=False):
        ...
        try:
            ...
            window_class = (
                window_class.split("=")[1].split(",")[0].strip('\n" ')
            )
        except IndexError:
            raise Exception(
    ...
    def open_file_and_check_viewer(
        self, filename, expected_app_titles, expected_app_classes, dispvm=False
    ):
        if dispvm:
            p = self.loop.run_until_complete(
                self.source_vm.run(
                    "qvm-open-in-dvm {}".format(filename),
        ...
        with self.qrexec_policy(
            "qubes.OpenInVM", self.source_vm.name, self.target_vmname
        ):
            with self.qrexec_policy(
                "qubes.OpenURL", self.source_vm.name, self.target_vmname
            ):
                while True:
                    ...
                    if retcode == 0:
                        ...
                        if (
                            window_title
                            and not window_title.startswith("LibreOffice")
                            and not window_title.startswith("NetworkManager")
        ...
        def check_matches(obj, patterns):
            return any(
                (
                    pat.search(obj)
                    if isinstance(pat, type(re.compile("")))
                    else pat in obj
                )
                for pat in patterns
            )
        ...
        if not check_matches(
            window_title, expected_app_titles
        ) and not check_matches(window_class, expected_app_classes):
            self.fail(
                "Opening file {} resulted in window '{} ({})', which is "
                "none of {!r} ({!r})".format(
                    filename,
                    window_title,
                    window_class,
    ...
    def prepare_txt(self, filename):
        self.loop.run_until_complete(
            self.source_vm.run_for_stdio(
                "cat > {}".format(filename), input=b"This is test\n"
            )
        )
    ...
    def prepare_pdf(self, filename):
        ...
        try:
            self.loop.run_until_complete(self.source_vm.run_for_stdio(cmd))
        except subprocess.CalledProcessError as e:
    ...
    def prepare_doc(self, filename):
        ...
        try:
            ...
            self.skipTest(
                "Failed to run '{}': {}".format(cmd, e.stderr.decode())
            )
    ...
    def test_101_pdf_dispvm(self):
        filename = "/home/user/test_file.pdf"
        self.prepare_pdf(filename)
        self.open_file_and_check_viewer(filename, [], ["evince"], dispvm=True)

    def test_102_doc_dispvm(self):
        filename = "/home/user/test_file.doc"
        self.prepare_doc(filename)
        self.open_file_and_check_viewer(
            filename, [], ["libreoffice", "abiword"], dispvm=True
        )
    ...
    def test_103_pptx_dispvm(self):
        ...
        self.prepare_pptx(filename)
        self.open_file_and_check_viewer(
            filename, [], ["libreoffice"], dispvm=True
        )
    ...
    def test_104_png_dispvm(self):
        ...
        self.prepare_png(filename)
        self.open_file_and_check_viewer(
            filename, [], ["shotwell", "eog", "display"], dispvm=True
        )
