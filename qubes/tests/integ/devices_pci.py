...
class TC_00_Devices_PCI(qubes.tests.SystemTestCase):
    def setUp(self):
        ...
        if self._testMethodName not in ["test_000_list"]:
            ...
            self.assignment = DeviceAssignment(self.dev, mode="required")
            if isinstance(self.dev, qubes.device_protocol.UnknownDevice):
                self.skipTest(
                    "Specified device {} does not exists".format(pcidev)
                )
    ...
    def test_000_list(self):
        ...
        if actual_devices:
            self.fail(
                "Not all devices listed, missing: {}".format(actual_devices)
            )
    ...
    def assertDeviceIs(
        ...
        self.assertTrue(
            required
            == device
            in dev_col.get_assigned_devices(required_only=True)
        )
    ...
    def test_011_attach_offline_temp_fail(self):
        ...
        with self.assertRaises(qubes.exc.QubesVMNotRunningError):
            self.loop.run_until_complete(dev_col.attach(self.assignment))
