...
class BackupTestsMixin(object):
    ...
    def create_backup_vms(self, pool=None):
        ...
        self.loop.run_until_complete(testnet.create_on_disk(pool=pool))
        ...
        testvm1 = self.app.add_new_vm(
            qubes.vm.appvm.AppVM, name=vmname, template=template, label="red"
        )
        testvm1.netvm = testnet
        self.loop.run_until_complete(testvm1.create_on_disk(pool=pool))
        vms.append(testvm1)
        ...
        self.loop.run_until_complete(testvm2.create_on_disk(pool=pool))
        self.fill_image_vm(testvm2, "root", 1024 * 1024 * 1024, True)
        vms.append(testvm2)

        vmname = self.make_vm_name("template")
        self.log.debug("Creating %s" % vmname)
        testvm3 = self.app.add_new_vm(
            qubes.vm.templatevm.TemplateVM, name=vmname, label="red"
        )
        self.loop.run_until_complete(testvm3.create_on_disk(pool=pool))
        ...
        testvm4 = self.app.add_new_vm(
            qubes.vm.appvm.AppVM, name=vmname, template=testvm3, label="red"
        )
        ...
        for vm in vms:
            ...
            for dev_class in vm.devices.keys():
                ...
                for ass in vm.devices[dev_class].get_assigned_devices():
                    vm_info["devices"][dev_class][str(ass)] = ass.options
            ...
            for dev_class in vm_info["devices"]:
                for dev in vm_info["devices"][dev_class]:
                    ...
                    for restored_dev_ass in restored_vm.devices[
                        ...
                        if str(restored_dev_ass) == dev:
                            found = True
                            self.assertEqual(
                                vm_info["devices"][dev_class][dev],
...
class TC_00_Backup(BackupTestsMixin, qubes.tests.SystemTestCase):
    ...
    def test_004_sparse_multipart(self):
        ...
        try:
            ...
            self.fill_image_vm(
                self.hvmtemplate, "private", 195 * 1024 * 1024 - 4096 * 3
            )
            self.fill_image_vm(
                self.hvmtemplate, "root", 1024 * 1024 * 1024, sparse=True
            )
    ...
    def test_010_selective_restore(self):
        ...
        exclude = [self.make_vm_name("test-net"), self.make_vm_name("template")]
        ...
        try:
            ...
            for vm_name in vms_info:
                if vm_name == self.make_vm_name("test1"):
                    ...
                    vms_info[vm_name]["properties"]["netvm"] = str(
    ...
    def test_020_encrypted_backup_non_ascii(self):
        ...
        try:
            orig_hashes = self.vm_checksum(vms)
            vms_info = self.get_vms_info(vms)
            self.make_backup(vms, passphrase="zażółć gęślą jaźń")
            self.remove_vms(reversed(vms))
            self.restore_backup(passphrase="zażółć gęślą jaźń")
            self.assertCorrectlyRestored(vms_info, orig_hashes)
        finally:
            del vms
...
class TC_10_BackupVMMixin(BackupTestsMixin):
    ...
    def test_100_send_to_vm_file_with_spaces(self):
        ...
        try:
            ...
            self.loop.run_until_complete(
                self.backupvm.run_for_stdio("mkdir '/var/tmp/backup directory'")
            )
            self.make_backup(
                vms,
            ...
            self.restore_backup(source=backup_path, appvm=self.backupvm)
            self.assertCorrectlyRestored(vms_info, orig_hashes)
        finally:
            del vms
    ...
    def test_110_send_to_vm_command(self):
        ...
        try:
            ...
            self.remove_vms(reversed(vms))
            self.restore_backup(
                source="dd if=/var/tmp/backup-test", appvm=self.backupvm
            )
            self.assertCorrectlyRestored(vms_info, orig_hashes)
        finally:
            del vms
