...
class SaltTestMixin(object):
    ...
    def salt_call(self, cmd):
        ...
        p = self.loop.run_until_complete(
            asyncio.create_subprocess_exec(*full_cmd, stdout=subprocess.PIPE)
        )
...
class TC_00_Dom0(SaltTestMixin, qubes.tests.SystemTestCase):
    def test_000_top_enable_disable(self):
        ...
        self.assertIn("local", cmd_output)
        self.assertNotIn(
            "test_salt.something", cmd_output["local"].get("base", [])
        )
    ...
    def test_001_state_sls(self):
        ...
        state_id = (
            "test_|-test-top-enable_|-test-top-enable_|-succeed_without_changes"
        )
        self.assertIn("local", cmd_output)
    ...
    def test_010_create_vm(self):
        ...
        self.assertEqual(
            state_out,
            {
                ...
                "changes": {
                    "qvm.prefs": {
                        "qvm.create": {"vcpus": {"new": 1, "old": "*default*"}}
                    },
                },
    ...
    def test_011_set_prefs(self):
        ...
        vm = self.app.add_new_vm("AppVM", label="red", name=vmname)
...
class SaltVMTestMixin(SaltTestMixin):
    ...
    def setUp(self):
        ...
        if self.id().endswith("user_sls"):
            if os.path.exists("/srv/user_salt"):
                shutil.copy(
                    "/srv/user_salt/top.sls", "/srv/user_salt/top.sls.test-bak"
                )
    ...
    def tearDown(self):
        if hasattr(self, "user_salt_exists"):
            if self.user_salt_exists:
                ...
                os.rename(
                    "/srv/user_salt/top.sls.test-bak", "/srv/user_salt/top.sls"
                )
            else:
    ...
    def test_000_simple_sls(self):
        ...
        self.assertTrue(
            state_output.startswith(expected_output),
        ...
        try:
            ...
            if end != len(json_data):
                warnings.warn(
                    f"Trailing junk in json output: {json_data[end:]}"
                )
        ...
        try:
            # or sls file
            self.assertEqual(
                state_output_json["__sls__"], "test_salt.something"
            )
        ...
        stdout, stderr = self.loop.run_until_complete(
            self.vm.run_for_stdio("cat /home/user/testfile")
        )
    ...
    def test_001_multi_state_highstate(self):
        ...
        for state in states:
            with open(
                os.path.join(self.salt_testdir, state + ".sls"), "w"
            ) as f:
                f.write("/home/user/{}:\n".format(state))
                f.write("  file.managed:\n")
                f.write("    - contents: |\n")
                f.write("        this is test\n")
            with open(
                os.path.join(self.salt_testdir, state + ".top"), "w"
            ) as f:
                f.write("base:\n")
                f.write("  {}:\n".format(vmname))
                f.write("    - test_salt.{}\n".format(state))
        ...
        for state in states:
            ...
            stdout, stderr = self.loop.run_until_complete(
                self.vm.run_for_stdio("cat /home/user/" + state)
            )
            self.assertEqual(stdout, b"this is test\n")
            self.assertEqual(stderr, b"")
        ...
        stdout, stderr = self.loop.run_until_complete(
            self.vm.run_for_stdio("cat /home/user/test2")
        )
    ...
    def test_002_grains_id(self):
        ...
        self.test_template = self.app.add_new_vm(
            "TemplateVM", name=tplname, label="red"
        )
        ...
        self.vm = self.app.add_new_vm(
            "AppVM", name=vmname, label="red", template=self.test_template
        )
        self.loop.run_until_complete(self.vm.create_on_disk())
        # test with some jinja templating
        ...
        for name, output in ((tplname, tpl_output), (vmname, appvm_output)):
            ...
            try:
                ...
                if end != len(output):
                    warnings.warn(
                        f"Trailing junk in json output: {output[end:]}"
                    )
            except json.decoder.JSONDecodeError as e:
                self.fail(
                    "JSON output decoding error: {}\n{}".format(e, output)
                )
            state_id = "file_|-/home/user/test2-{0}_|-/home/user/test2-{0}_|-managed".format(
                name
            )
            # drop the header
    ...
    def test_003_update(self):
        ...
        for state, result in state_output_json[vmname].items():
            self.assertTrue(
                result["result"], "State {} failed: {!r}".format(state, result)
            )
