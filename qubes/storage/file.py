...
EMPTY_SNAPSHOT = (
    b"SnAp\x01\x00\x00\x00\x01\x00\x00\x00\x00\x01\x00\x00" + bytes(262128)
)
...
class FilePool(qubes.storage.Pool):
    ...
    def __init__(
        self, *, name, revisions_to_keep=1, dir_path, ephemeral_volatile=False
    ):
        super().__init__(
            name=name,
    ...
    def init_volume(self, vm, volume_config):
        if volume_config.get("snap_on_start", False) and volume_config.get(
            "save_on_stop", False
        ):
...
class FileVolume(qubes.storage.Volume):
        ...
        if self.save_on_stop:
            with suppress(FileNotFoundError), open(self.path_cow, "rb") as cow:
                ...
                return has_any_data(cow) and (
                    cow_used > len(EMPTY_SNAPSHOT)
                    or cow.read(len(EMPTY_SNAPSHOT)) != EMPTY_SNAPSHOT
                    or cow_used > cow.seek(0, os.SEEK_HOLE)
                )
        return False
    ...
    def resize(self, size):  # pylint: disable=invalid-overridden-method
        ...
        with subprocess.Popen(
            ["losetup", "--associated", self.path], stdout=subprocess.PIPE
        ) as p:
        ...
        if m is not None:
            ...
            # resize loop device
            subprocess.check_call(["losetup", "--set-capacity", loop_dev])
            if self.save_on_stop:
    ...
    def reset(self):
        ...
        assert (
            not self.snap_on_start and not self.save_on_stop
...
def copy_file(source, destination):
    ...
    assert os.path.exists(source), "Missing the source %s to copy from" % source
    ...
    try:
        cmd = ["cp", "--sparse=always", "--reflink=auto", source, destination]
        subprocess.check_call(cmd)
    except subprocess.CalledProcessError:
        raise IOError(
            "Error while copying {!r} to {!r}".format(source, destination)
        )
