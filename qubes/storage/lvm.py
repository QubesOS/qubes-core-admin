...
class ThinPool(qubes.storage.Pool):
    ...
    *volume_group*/*thin_pool* arguments. LVM volume naming scheme:
    ...
    def __repr__(self):
        return (
            "<{} at {:#x} name={!r} volume_group={!r} thin_pool={!r}>".format(
                type(self).__name__,
                id(self),
                self.name,
                self.volume_group,
...
class ThinVolume(qubes.storage.Volume):
    ...
    def path(self):
        return "/dev/mapper/" + (
            self._vid_current.replace("-", "--").replace("/", "-")
        )
    ...
    def _vid_current(self):
        ...
        if vol_revisions:
            last_revision = max(vol_revisions.items(), key=_revision_sort_key)[
                0
            ]
    ...
    def size(self):
        try:
            if (self.snap_on_start or self.save_on_stop) and os.path.exists(
                "/dev/" + self._vid_snap
            ):
    ...
    async def _commit(self, vid_to_commit=None, keep=False):
        ...
        cmd = ["clone" if keep else "rename", vid_to_commit, self.vid]
        await qubes_lvm_coro(cmd, self.log)
        await reset_cache_coro()
    ...
    async def import_volume(self, src_volume):
        ...
        if (
            hasattr(src_volume.pool, "thin_pool")
            and src_volume.pool.thin_pool == self.pool.thin_pool
        ):  # NOQA
            # pylint: disable=protected-access
            ...
            try:
                ...
                await qubes.utils.coro_maybe(src_volume.export_end(src_path))
            if p.returncode != 0:
                cmd = ["remove", self._vid_import]
                await qubes_lvm_coro(cmd, self.log)
                raise qubes.exc.StoragePoolException(
    ...
    def is_outdated(self):
        ...
        # pylint: disable=protected-access
        return (
            size_cache[self._vid_snap]["origin"]
    ...
    def encrypted_volume_path(self, qube_name, device_name):
        ...
        # pylint: disable=line-too-long
        vid = "-".join(
            i.replace("-", "--") for i in self._vid_current.split("/")
        )
