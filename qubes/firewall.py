...
class Expire(RuleOption):
    def __init__(self, untrusted_value):
        super().__init__(untrusted_value)
        if not (
            isinstance(untrusted_value, (int, float))
            or untrusted_value.isdigit()
        ):
...
class Rule(qubes.PropertyHolder):
    def __init__(self, xml=None, **kwargs):
        ...
        if self.dstports:
            self.on_set_dstports(
                "property-set:dstports", "dstports", self.dstports, None
            )
        if self.icmptype:
            self.on_set_icmptype(
                "property-set:icmptype", "icmptype", self.icmptype, None
            )
        self.property_require("action", False, True)

    action = qubes.property("action", type=Action, order=0, doc="rule action")

    proto = qubes.property(
        "proto", type=Proto, default=None, order=1, doc="protocol to match"
    )

    dsthost = qubes.property(
        "dsthost",
        type=DstHost,
        default=None,
        order=1,
    ...
    comment = qubes.property(
        "comment", type=Comment, default=None, doc="User comment"
    )
    ...
    def api_rule(self):
        ...
        for prop in sorted(
            self.property_list(), key=lambda p: p.__name__ == "comment"
        ):
            value = getattr(self, prop.__name__)
            if value is None:
                continue
        ...
        for untrusted_option in untrusted_options.strip().split(" "):
            ...
            if untrusted_key in [str(prop) for prop in cls.property_list()]:
                kwargs[untrusted_key] = cls.property_get_def(
                    untrusted_key
                ).type(untrusted_value=untrusted_value)
...
class Firewall:
        ...
        for rule in self.rules:
            if rule.expire:
                if rule.expire and rule.expire.expired:
                    continue
                if (
                    nearest_expire is None
                    or rule.expire.datetime < nearest_expire
                ):
    ...
    def qdb_entries(self, addr_family=None):
        ...
        None, include both
        """
        entries = {"policy": str(self.policy)}
